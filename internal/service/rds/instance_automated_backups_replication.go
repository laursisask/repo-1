...
func resourceInstanceAutomatedBackupsReplicationDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if sourceDatabaseARN.Region != meta.(*conns.AWSClient).Region {
		session, err := conns.NewSessionForRegion(&conn.Config, sourceDatabaseARN.Region, meta.(*conns.AWSClient).TerraformVersion)
		if err != nil {
			return fmt.Errorf("error creating new client session for region (%s): %w", sourceDatabaseARN.Region, err)
		}

		sourceDatabaseConn = rds.New(session)
	}
